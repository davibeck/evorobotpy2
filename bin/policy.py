...
class Policy(object):
    def __init__(self, env, fileini, seed, test):
        ...
        self.displayneurons = (
            0  # Gym policies can display or the robot or the neurons activations
        )
        ...
        self.ac = np.arange(
            self.noutputs * self.nrobots, dtype=np.float32
        )  # allocate action vector
        if self.nbins == 1:  # allocate neuron activation vector
            self.nact = np.arange(
                (self.ninputs + (self.nhiddens * self.nlayers) + self.noutputs)
                * self.nrobots,
                dtype=np.float64,
            )
        else:
            self.nact = np.arange(
                (
                    self.ninputs
                    + (self.nhiddens * self.nlayers)
                    + (self.noutputs * self.nbins)
                )
                * self.nrobots,
                dtype=np.float64,
            )
        ...
        self.normphase = (
            0  # whether we collect data to update normalization in the current episode
        )
        self.norm_prob = (
            0.01  # the fraction of episodes in which normalization data is collected
        )
        self.nn.copyGenotype(
            self.params
        )  # pass the pointer to the parameters to evonet
        self.nn.copyInput(
            self.ob
        )  # pass the pointer to the observation vector to evonet
        self.nn.copyOutput(self.ac)  # pass the pointer to the action vector to evonet
        self.nn.copyNeuronact(
            self.nact
        )  # pass the pointer to the neuron activation vector to evonet
        if (
            self.normalize == 1
        ):  # pass the pointer to the nornalization vector to evonet
    ...
    def set_trainable_flat(self, x):
        ...
        self.nn.copyGenotype(
            self.params
        )  # copy a vector of parameters in the evonet parameter vector
        if self.nmorphparams > 0:
            self.env.setParams(
                self.params[-self.nmorphparams :]
            )  # set the morphology with the morphological parameters
    ...
    def readConfig(
        self,
    ):  # load hyperparameters from the [POLICY] section of the ini file
...
class BulletPolicy(Policy):
    ...
    def rollout(
        ...
        for trial in range(ntrials):
            ...
            rew = 0
            t = 0
            while t < nicheMaxsteps:
                self.nn.copyInput(
                    self.ob
                )  # copy the pointer to the observation vector to evonet
...
class GymPolicyDiscr(Policy):
    def __init__(self, env, filename, seed, test):
        ...
        self.noutputs = (
            env.action_space.n
        )  # only works for problems with discrete action space
        Policy.__init__(self, env, filename, seed, test)
...
class ErPolicy(Policy):
    def __init__(self, env, filename, seed, test):
        ...
        env.copyObs(
            self.ob
        )  # pass the pointer to the observation vector to the Er environment
        env.copyAct(
            self.ac
        )  # pass the pointer to the action vector to the Er environment
        env.copyDone(
            self.done
