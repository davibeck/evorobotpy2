...
if len(sys.argv) == 1:
    ...
    aves = [ave_esne, ave_es, ave_ne, ave_sss, ave_cmaes, ave_cmane, ave_esnedavi]
    label = [
        "OpenAI-ES-NE",
        "OpenAI-ES",
        "SSS-NE",
    ...
    for ave in aves:
        midx = [
            np.mean([ave[file][0][j] for file in range(len(ave[:]))])
            for j in range(100)
        ]
        midy = [
            np.mean([ave[file][1][j] for file in range(len(ave[:]))])
            for j in range(100)
        ]
