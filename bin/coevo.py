...
class Algo(EvoAlgo):
    ...
    def run(self):
        ...
        print(
            "Coevo seed %d Popsize %d %d batchSize %d stepsize %lf noiseStdDev %lf wdecay %d nparams %d"
            % (
                self.seed,
                self.popsize,
                self.selsize,
        ...
        for gen in range(self.ngenerations):
            ...
            for i1 in range(self.selsize):
                for i2 in range(self.popsize):
                    self.policy.set_trainable_flat(
                        np.concatenate((self.selp[i1], pop[i2]))
                    )
                    eval_rews, eval_length = self.policy.rollout(1)
            ...
            fm = self.fmatrix[0 : self.popsize + self.selsize, 0 : self.popsize].mean(
                axis=1, dtype="float"
            )
            ...
            for i1 in range(maxi):
                ...
                for i2 in range(maxi):
                    if rendt:
                        print(
                            "robot1 %d robot2 %d "
                            % (rank1[popsize - i1 - 1], rank1[popsize - i2 - 1]),
                            end="",
                        )
                    self.policy.set_trainable_flat(
                        np.concatenate(
                            (pop[rank1[popsize - i1 - 1]], pop[rank1[popsize - i2 - 1]])
                        )
                    )
                    eval_rews, eval_length = self.policy.rollout(1)
                    tot_rew += eval_rews
                    ...
                    if not rendt:
                        print("%.2f " % eval_rews, end="")
                    if (
                        fmatrix[rank1[popsize - i1 - 1]][rank1[popsize - i2 - 1]]
                        != eval_rews
                    ):
                        print(
                            "warning [%.2f %.2f]"
            ...
            for pp in range(2):
                ...
                for i1 in range(psizea):
                    for i2 in range(psizeb):
                        if pp == 0:
                            self.policy.set_trainable_flat(
                                np.concatenate((pop1[i1], pop1[i2]))
                            )
                        if pp == 1:
                            self.policy.set_trainable_flat(
                                np.concatenate((pop1[i1], pop2[i2]))
                            )
                        rew, eval_length = self.policy.rollout(1)
                        tot_rew[pp] += rew
