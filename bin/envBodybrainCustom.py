...
class ContactDetector(contactListener):
    ...
    def BeginContact(self, contact):
        if (
            self.env.torso == contact.fixtureA.body
            or self.env.torso == contact.fixtureB.body
        ):
...
class customEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.njoints = (
            self.numSegs
        )  # number of joints matches number of segments (there is one joint between each pair of segments including torso)
        self.nparams = (
            self.nsizes + self.nangleranges + self.nangles + self.ndensities
        )  # Torso width and height + 2 for each segment (width and height) + 2 for each joint (asymmetric angular ranges) except for the head + torso angle + one for each segment angle + torso density + one for each segment density. N.B.: number of joints matches number of segments (there is one joint between each pair of segments including torso)
        self.params = np.zeros(
            self.nparams, dtype=np.float64
        )  # np.ones(self.nparams, dtype=np.float64)
        self.factors = np.ones(
            self.nparams, dtype=np.float64
        )  # 2 for each segment (width and height) + 2 for each joint (asymmetric angular ranges) + torso angle + segment angle
        self.rate = 0.0
        ...
        ob_len = (
            5 + (self.njoints * 2) + self.numSegs
        )  # 5 for torso, njoints * 2 for each joint, contact flag for each segment
        # Number of actions
        ...
        for i in range(TERRAIN_LENGTH):
            ...
            if state == GRASS and not oneshot:
                ...
                if i > TERRAIN_STARTPAD:
                    velocity += (
                        self.np_random.uniform(-1, 1) / SCALE * TERRAIN_STEPNESS
                    )  # 1
                y += velocity
    ...
    def reset(self):
        ...
        torsoAngle = self.computeFactor(
            (3.0 * math.pi / 4.0), (self.nsizes + self.nangleranges)
        )
        ...
        self.torso = self.world.CreateDynamicBody(
            ...
            fixtures=fixtureDef(
                shape=polygonShape(
                    box=(torsoWidth / 2, torsoHeight / 2)
                ),  # Width and height inverted (see rendering)
                density=self.computeFactor(
                    self.segDensity, self.nsizes + self.nangleranges + self.nangles
                ),  # Density can be increased/decreased up to 80%
                restitution=0.0,
                categoryBits=0x0020,
                maskBits=0x001,
            ),
        ...
        tmpX = init_x - torsoWidth / 2.0 * math.cos(
            torsoAngle
        )  # - torsoHeight / 2.0 * math.sin(torsoAngle)
        tmpY = init_y - torsoWidth / 2.0 * math.sin(
            torsoAngle
        )  # - torsoHeight / 2.0 * math.cos(torsoAngle)
        for i in range(self.numSegs):
            # New sizes
            ...
            seg = self.world.CreateDynamicBody(
                ...
                position=(
                    tmpX - currWidth / 2.0 * math.cos(currAngle),
                    tmpY - currWidth / 2.0 * math.sin(currAngle),
                ),
                angle=currAngle,
                fixtures=fixtureDef(
            ...
            # Set point coords
            tmpX = tmpX - currWidth * math.cos(
                currAngle
            )  # - currHeight * math.sin(currAngle)
            tmpY = tmpY - currWidth * math.sin(
                currAngle
            )  # - currHeight * math.cos(currAngle)
            # Update indices
            k += 2
            h += 2
            z += 1
    ...
    def step(self, action):  # _step
        ...
        for i in range(len(action)):
            ...
            self.joints[j].maxMotorTorque = float(
