...
def parseConfigFile(filename):
    ...
    if os.path.isfile(filename):
        ...
        for o in options:
            ...
            if found == 0:
                print(
                    "\033[1mOption %s in section [EXP] of %s file is unknown\033[0m"
                    % (o, filename)
                )
                sys.exit()
    else:
...
def helper():
    ...
    print(
        "-f [fileini]              : the file containing the hyperparameters(mandatory)"
    )
    print("-s [integer]              : the number used to initialize the seed")
    print("-n [integer]              : the number of replications to be run")
    print(
        "-p                        : the flag to be used to postevaluate an agent (see parameter -g)"
    )
    print(
        "-g [filename]             : the name of teh file containing the parameters of the agent"
    )
    print(
        "-d                        : the flag that permit to show the activation of neurons during postevaluation"
    )
    print(
        "-o [directory]            : the directory where all output files are stored (default current dir)"
    )
    print(
        "-w [integer]              : the number of workers used by parallel implementations"
    )
    print("")
    ...
    print(
        "algo [string]             : adaptive algorithm (default 'OpenAI-ES'), SSS, coevo, coevo2"
    )
    print("[POLICY]")
    ...
    print(
        "action_noise [0/1/2]      : action noise 0=none, 1=gaussian 2=diagonal gaussian (default 0)"
    )
...
def main(argv):
    ...
    parser.add_argument(
        "-f", "--fileini", help="name of ini file", type=str, default=""
    )
    parser.add_argument(
        "-s", "--seed", help="random generator seed", type=int, default=1
    )
    parser.add_argument(
        "-n", "--nreplications", help="number of replications", type=int, default=1
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-g",
        "--genofile",
        help="name of the file containing the parameters of the agent",
        type=str,
        default=None,
    )
    parser.add_argument(
        "-d",
        "--displayneurons",
        help="show the activation of the neurons",
        action="store_true",
        default=False,
    )
    ...
    if (
        len(args.fileini) == 0
    ):  # if the name of the inifile is not specified display help information
    ...
    if (
        "Er" in environment
    ):  # Er environment (implemented in C++ and wrapped with Cython)
        ...
        if isinstance(env.action_space, gym.spaces.box.Box):
            ...
            policy = GymPolicy(
                env, args.fileini, args.seed, test
            )  # with continuous action space
        else:
            from policy import GymPolicyDiscr

            policy = GymPolicyDiscr(
                env, args.fileini, args.seed, test
