...
class Algo(EvoAlgo):
    ...
    def inniche(self, niche):
        ...
        # mutate offspring
        aux = np.array(
            self.pop[bestg] + self.rg.randn(1, self.policy.nparams) * self.mutation
        )
        offspring = aux
        ...
        for niche in range(self.number_niches):
            ...
            for miche in range(self.number_niches):
                if miche != niche:
                    ...
                    self.policy.set_trainable_flat(
                        self.pop[self.boffspring[niche]]
    ...
    def run(self):
        ...
        self.niches = [
            [
                random.randint(1, self.number_niches * 10**5)
                for _ in range(self.policy.ntrials)
            ]
            for _ in range(self.number_niches)
        ]
