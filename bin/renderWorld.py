...
class Base(object):
    ...
    def __init__(self, x, y, z=0, color=(0.0, 0.0, 0.0, 1.0), stroke=0, rotation=0):
        try:
            self.rect
...
class Circle(Base):
    ...
    def render(self):
        ...
        gluDisk(
            self.q, inner, self.radius, self.circleresolution, 1
...
class Arc(Base):
    ...
    def render(self):
        ...
        gluPartialDisk(
            self.q, inner, self.radius, self.circleresolution, 1, self.start, self.sweep
        )
...
class Polygon(Base):
    def __init__(self, v, z=0, color=(0, 0, 0, 1), stroke=0, rotation=0.0, style=0):
        ...
        self.v2 = [
            (i[0] - self.rect.x, i[1] - self.rect.y) for i in v
        ]  # relative polygon
    ...
    def drawVertex(
        self, x, y, z=0, v=(), color=(0, 0, 0, 1), stroke=0, rotation=0.0, style=0
    ):
        glColor4f(*self.color)
        glPushMatrix()
        ...
        if self.stroke:  # outlined polygon
            ...
            if len(v) == 4:
                ...
                glBegin(GL_TRIANGLES)  # which type of polygon are we drawing?
...
class Rect(object):
    ...
    def setQuad(self, q):  # [ q[0][0], q[0][1], q[1][0], q[2][1] ] # l t r b
        self.rect = (
            q[0][0] + (q[1][0] - q[0][0]) * 0.5,
            q[0][1] + (q[2][1] - q[0][1]) * 0.5,
            q[1][0] - q[0][0],
            q[2][1] - q[0][1],
        )
    ...
    def getLoc(self):
        return self.__x, self.__y  # self.x, self.y
...
def update(wobj, info, ob, ac, nact):
    ...
    while wobj[c] > 0.0:
        ...
        if wobj[c] == 1.0:
            o = Circle(
                ...
                wobj[c + 2],
                width=wobj[c + 3] * 2.0,
                color=(wobj[c + 5], wobj[c + 6], wobj[c + 7], 1.0),
            )
            o.render()
            o = Line(
                (wobj[c + 1], wobj[c + 2]),
    ...
    label = pyglet.text.Label(
        info, font_name="Arial", font_size=10, color=(0, 0, 0, 255), x=0, y=490
    )
