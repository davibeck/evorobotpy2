...
class Algo(EvoAlgo):
        ...
        if os.path.isfile(self.fileini):
            ...
            for o in options:
                ...
                if found == 0:
                    ...
                    print(
                        "ngenerations [integer]    : max number of generations (default 200)"
                    )
                    print("popsize [integer]         : popsize (default 40)")
                    print(
                        "selsize [integer]         : number selected agents (default 10)"
                    )
                    print(
                    ...
                    print(
                        "saveeach [integer]        : save file every N generations (default 100)"
                    )
    ...
    def run(self):
        ...
        nparams = int(
            self.policy.nparams / 2
        )  # parameters required for a single individul
        ...
        self.candidate = np.arange(
            nparams, dtype=np.float64
        )  # the vector containing varied parameters
        self.fmatrix = np.zeros(
            (self.popsize + self.selsize, self.popsize + +self.selsize),
            dtype=np.float64,
        )  # the fitness of each individual against each competitor
        # the additional lines and columns contain data of evolving individuals

        self.selp = np.arange(
            nparams * self.selsize, dtype=np.float64
        )  # the parameters of the selected individuals
        self.selm = np.arange(
            nparams * self.selsize, dtype=np.float64
        )  # the momentum of the selected individuals
        self.selv = np.arange(
            nparams * self.selsize, dtype=np.float64
        )  # the squared-momentum of the selected individuals
        self.selcomp = np.arange(
            nparams * self.selsize, dtype=np.float64
        )  # the parameters of the selected competitors
        ...
        print(
            "Coevo2 seed %d Popsize %d %d batchSize %d stepsize %lf noiseStdDev %lf wdecay %d nparams %d"
            % (
                self.seed,
                self.popsize,
                self.selsize,
        ...
        for i1 in range(self.popsize):
            for i2 in range(self.popsize):
                self.policy.set_trainable_flat(
                    np.concatenate((pop[i1], pop[self.popsize + i2]))
                )
        ...
        for gen in range(self.ngenerations):
            ...
            for i1 in range(self.selsize):
                for i2 in range(self.popsize):
                    if self.evopop == 0:
                        self.policy.set_trainable_flat(
                            np.concatenate((self.selp[i1], pop[self.popsize + i2]))
                        )
                    else:
                        self.policy.set_trainable_flat(
                            np.concatenate((pop[i2], self.selp[i1]))
                        )
                    eval_rews, eval_length = self.policy.rollout(1)
                    if self.evopop == 0:
                        self.fmatrix[self.popsize + i1][
                            i2
                        ] = eval_rews  # additional rows of the popsize*popsize matrix
                    else:
                        self.fmatrix[i2][
                            self.popsize + i1
                        ] = eval_rews  # additional columns of the popsize*popsize matrix
            if self.evopop == 0:
                # average lines (pop 1), submatrix of popsize+selsize raws and popsize columns
                fm = self.fmatrix[
                    0 : self.popsize + self.selsize, 0 : self.popsize
                ...
                fm = self.fmatrix[
                    0 : self.popsize, 0 : self.popsize + self.selsize
            ...
            for i in range(self.popsize):
                if (
                    orderfm[i + self.selsize] >= self.popsize
                ):  # evolving individual ranked among the best
                    evoi = orderfm[i + self.selsize] - self.popsize
                    worsei = orderfm[replaced]
                    print(
                        "%d->%d %.2f "
                        % (evoi, worsei, fm[self.popsize + evoi] - fm[worsei]),
                        end="",
                    )
                    if self.evopop == 0:
                        for p in range(nparams):
                        ...
                        for c in range(self.popsize):
                            self.fmatrix[worsei][c] = self.fmatrix[evoi + self.popsize][
                                c
                            ]
                    else:
                        for p in range(nparams):
                        ...
                        localprog += fm[self.popsize + evoi] - fm[worsei]
                        for c in range(self.popsize):
                            self.fmatrix[c][worsei] = self.fmatrix[c][
                                evoi + self.popsize
                            ]
                    replaced += 1
                    while orderfm[replaced] >= self.popsize and replaced < (
                        self.popsize - 1
                    ):
                        replaced += 1
            ...
            print(
                "seed %d gen %d popfit %.2f %.2f weights %.2f"
                % (seed, gen, fm, 1.0 - fm, np.average(np.absolute(pop)))
            )
            ...
            for i1 in range(len(unselected)):
                ...
                for i2 in range(len(selected)):
                    for i3 in range(self.popsize):
                        if self.evopop == 0:
                            diff += abs(
                                self.fmatrix[i3][unselected[i1]]
                                - self.fmatrix[i3][selected[i2]]
                            )
                        else:
                            diff += abs(
                                self.fmatrix[unselected[i1]][i3]
                                - self.fmatrix[selected[i2]][i3]
                            )
                if diff > maxdiff:
                    selind = i1
                    maxdiff = diff
    ...
    def runphase(self, sind, nparams):
        ...
        for it in range(20):
            ...
            for i in range(self.selsize):
                if self.evopop == 0:
                    self.policy.set_trainable_flat(
                        np.concatenate((self.selp[sind], self.selcomp[i]))
                    )
                    eval_rews, eval_length = self.policy.rollout(1)
                    # sanity check
                    if (
                        it == 0
                        and eval_rews != self.fmatrix[self.seli[sind], self.selc[i]]
                    ):
                        print("warning: sanity check failed")
                    ave_rews += eval_rews
                else:
                    self.policy.set_trainable_flat(
                        np.concatenate((self.selcomp[i], self.selp[sind]))
                    )
                    eval_rews, eval_length = self.policy.rollout(1)
                    # sanity check
                    if (
                        it == 0
                        and eval_rews != self.fmatrix[self.selc[i], self.seli[sind]]
                    ):
                        print("warning: sanity check failed")
                ...
                for bb in range(2):
                    if bb == 0:
                        for g in range(nparams):
                            self.candidate[g] = (
                                self.selp[sind][g] + samples[b, g] * self.noiseStdDev
                            )
                    else:
                        for g in range(nparams):
                            self.candidate[g] = (
                    ...
                    for c in range(self.selsize):
                        if self.evopop == 0:
                            self.policy.set_trainable_flat(
                                np.concatenate((self.candidate, self.selcomp[c]))
                            )
                            eval_rews, eval_length = self.policy.rollout(1)
                            ave_rews += eval_rews
                        else:
                            self.policy.set_trainable_flat(
                                np.concatenate((self.selcomp[c], self.candidate))
                            )
                            eval_rews, eval_length = self.policy.rollout(1)
                            ave_rews += 1.0 - eval_rews
            ...
            while i < self.batchSize:
                ...
                if self.batchSize - i < 500:
                    ...
                    gsize = 500
                g += dot(
                    weights[i : i + gsize], samples[i : i + gsize, :]
                )  # weights * samples
                i += gsize
            ...
            self.selv[sind] = beta2 * self.selv[sind] + (1.0 - beta2) * (
                globalg * globalg
            )
    ...
    def testusage(self):
        print(
            "ERROR: To post-evaluate with the coevo algorithm you should specify with the -g parameter a string containing:"
        )
        print(
            "P-ng-ni (postevaluate a population) where ng is generation number and ni the number of best agents to be posteveluated"
        )
        print("p-ng-ni (postevaluate a population without displaying the behavior)")
        print(
            "m-ng-ngg (master tournament) where ng is the last generation number and ngg is the generation interval "
        )
        print(
            "c-pop1-pop2 (population cross test) pop1 and pop2 are the name of the files containing the populations to be cross-tested "
        )
        sys.exit()
    ...
    def test(self, testparam):
        ...
        if parsen[0] == "p" or parsen[0] == "P":
            ...
            while ii1 < maxi:
                ...
                for i2 in range(maxi):
                    ...
                    self.policy.set_trainable_flat(
                        np.concatenate((pop[rank1[i1]], pop[popsize + rank2[i2]]))
                    )
                    eval_rews, eval_length = self.policy.rollout(1)
                    tot_rew += eval_rews
                    fitcol[i2] += eval_rews
                    ...
                    if fmatrix[rank1[i1]][rank2[i2]] != eval_rews:
                        print(
                            "warning [%.2f %.2f]"
                            % (eval_rews, fmatrix[rank1[i1]][rank2[i2]]),
                            end="",
                        )
                if not rendt:
        ...
        if parsen[0] == "m":
            ...
            bestrew2 = ""
            print(
                "seed %d: postevaluation gen %d against contemporary and ancient competitors every %d generaions"
                % (seed, int(parsen[1]), int(parsen[2]))
            )
            for pp in range(2):
                if pp == 0:
                ...
                while cgen >= 0:
                    ...
                    for i1 in range(popsize):
                        ...
                        for i2 in range(popsize):
                            if pp == 0:
                                self.policy.set_trainable_flat(
                                    np.concatenate((pop[i1], pop2[popsize + i2]))
                                )
                            else:
                                self.policy.set_trainable_flat(
                                    np.concatenate((pop2[i1], pop[popsize + i2]))
                                )
                    ...
                    if pp == 0:
                        print(
                            "%.2f " % (tot_rew / (popsize * popsize)),
                            end="",
                            flush=True,
                        )
                    else:
                        print(
                            "%.2f " % (1.0 - (tot_rew / (popsize * popsize))),
                            end="",
                            flush=True,
                        )
                    if pp == 0:
        ...
        if parsen[0] == "c" or parsen[0] == "C":
            ...
            assert (
                popshape1[1] == popshape2[1]
            ), "the number of parameters in the two file is inconsistent"
            ...
            for pp in range(4):
                ...
                for i1 in range(psizea):
                    for i2 in range(psizeb):
                        if pp == 0:
                            self.policy.set_trainable_flat(
                                np.concatenate((pop1[i1], pop1[popsize1 + i2]))
                            )
                        if pp == 1:
                            self.policy.set_trainable_flat(
                                np.concatenate((pop1[i1], pop2[popsize2 + i2]))
                            )
                        if pp == 2:
                            self.policy.set_trainable_flat(
                                np.concatenate((pop2[i1], pop1[popsize1 + i2]))
                            )
                        if pp == 3:
                            self.policy.set_trainable_flat(
                                np.concatenate((pop2[i1], pop2[popsize2 + i2]))
                            )
                        rew, eval_length = self.policy.rollout(1)
                        tot_rew[pp] += rew
            ...
            print(
                "tot  diff: %.2f"
                % (tot_rew[1] - tot_rew[0] + (1.0 - tot_rew[2]) - (1.0 - tot_rew[3]))
            )
