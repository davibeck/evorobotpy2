...
class Algo(EvoAlgo):
    ...
    def setProcess(self, n_workers, comm, rank):
        ...
        self.n_worker_samples = int(
            self.batchSize / (self.n_workers - 1)
        )  # number of sample evaluated by each worker
        self.id = self.rank - 1  # id of the process (master has id -1)
        self.cgen = 0  # currrent generation
        self.fitness = ones(
            self.n_workers * (self.n_worker_samples * 2)
        )  # vector of fitness filled by the master and the workers
        self.evals = zeros(
            self.n_workers, dtype=np.int32
        )  # vector of evaluation steps filled by the master and by the workers
        ...
        if self.policy.normalize == 1:  # normalization vector
            self.normvector = np.arange(
                self.n_workers * (self.policy.ninputs * 2), dtype=np.float64
            )  # normalization vector broadcasted to workers
        self.inormepisodes = (
    ...
    def evaluate(self):
        ...
        if self.rank == 0:  # Master: postevaluate the best sample of last generation
            ...
            ):  # Worker (evaluate a fraction of the population)
                for bb in range(2):
                    if bb == 0:
                        candidate = (
                            self.center
                            + self.samples[(self.id * self.n_worker_samples) + b, :]
                            * self.noiseStdDev
                        )
                    else:
                        candidate = (
                    ...
                    self.policy.nn.normphase(
                        0
                    )  # workers never collect normalization data
                    eval_rews, eval_length = self.policy.rollout(
                        self.policy.ntrials,
                        seed=(
                            self.seed
                            + (self.cgen * self.batchSize)
                            + (self.id * self.n_worker_samples)
                            + b
                        ),
                    )
    ...
    def update_normvector(self):
        if (
            self.rank > 0
        ):  # workers overwrite their normalization vector with the vector received from the master
    ...
    def run(self):
        ...
        while self.steps < self.maxsteps:
            (
                ...
                self.evaluate()
            )  # evaluate sample (each worker evaluate a fraction of the population)
            self.comm.Allgatherv(
                fitness_worker, [self.fitness, MPI.DOUBLE]
            )  # brodcast fitness value to all workers
            self.comm.Allgatherv(
                weval, [self.evals, MPI.INT]
            )  # broadcast number of steps performed to all workers
            if self.policy.normalize == 1:
                self.comm.Allgatherv(
                    self.policy.normvector, [self.normvector, MPI.DOUBLE]
                )  # broadcast normalization vector (it is update from the master, the vectors of the workers are ignored)

            self.samplefitness = self.fitness[
                (self.n_worker_samples * 2) :
            ]  # Merge the fitness of the workers by discarding the vector returned by the master
            self.steps += np.sum(
                self.evals
            )  # Update the total number of steps performed so far
            ...
            if self.rank == 0 and (
                (time.time() - last_save_time) > (self.saveeach * 60)
            ):
