...
"""
   ...
   Brenda S. Machado, brenda.silva.machado@grad.ufsc.br, and Arthur H. Bianchini, arthur.h.bianchini@grad.ufsc.br,
...
class Algo(EvoAlgo):
        ...
        if os.path.isfile(self.fileini):
            ...
            for o in options:
                ...
                if o == "maxmsteps":
                    ...
                    found = 1
                if o == "numgens":
                    self.numgens = config.getint("ALGO", "numgens")
                    found = 1
                if o == "stepsize":
    ...
    def setProcess(self):
        ...
        self.centers = [
            np.copy(self.policy.get_trainable_flat()) for i in range(self.number_niches)
    ...
    def savedata(self):
        self.save(
            esne=True
        )  # save the best agent so far, the best postevaluated agent so far, and progress data across generations
    ...
    def evaluate(self, niche, oniche=None):
        ...
        for b in range(self.batchSize):
            for bb in range(2):
                if bb == 0:
                    candidate = (
                        self.centers[niche] + self.samples[b, :] * self.noiseStdDev
                    )
                else:
                    candidate = (
                        self.centers[niche] - self.samples[b, :] * self.noiseStdDev
    ...
    def nicheMaxstep(self, index):
        nicheMaxstep = (self.policy.maxsteps / self.number_niches) * (index + 1)
        return nicheMaxstep
    ...
    def intraniche(self):
        for niche in range(self.number_niches):
            ...
            self.optimize(
                niche
            )  # estimate the gradient and move the centroid in the gradient direction
        """print(
    ...
    def run(self):
        ...
        for _ in range(num_random_niches):
            ...
            random_niches.append(
                # Coloca no array
                [
                    # Mudar isso para cada nicho ter o mesmo valor ou colocar ele pra baixo pra cada nicho tem um valor diferente a cada geração
