# evorobotpy2 version customized for evolutionary strategies
# include the calculation of the self.angle_to_target variable (user for the humanoid)
# reduced variation of the initial humanoid posture to avoid the compenetration of body parts
...
class Humanoid(WalkerBase):
    ...
    def robot_specific_reset(self, bullet_client):
        # WalkerBase.robot_specific_reset(self, bullet_client)
        # random range reduced to avoid initial compenetration
        # the next 8 lines replace WalkerBase.robot_specific_reset
        self._p = bullet_client
        for j in self.ordered_joints:
